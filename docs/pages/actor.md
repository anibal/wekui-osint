...
- An **Actor** is whoever or whatever performed a deliberate act: a **person** or an **agent**. An agent is a machine worker driven by a model and a prompt.
- Wherever this vocabulary says that something was approved, proposed, judged or retired, an Actor did it, and we record which one.
- Two agents are the same agent exactly when they share the same model and the same prompt; a different model, or a different prompt, is a different agent — so the record of what an agent did always points at one exact model-and-prompt pairing.
