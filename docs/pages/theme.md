...
- Whether a given [[post]] is about a Theme is a separate judgment, and it is now WRITTEN. The extractor answers it for every post it reads: the theme(s) the post is about, or — through `ThemeNone` — that it is about none. Both are ordinary judgments on the [[judgment]] spine, append-only and superseding, and together they are what makes *"every post is accounted for"* a fact the record holds rather than a line in a run summary that dies with the run.
  - Three things a post can be: it evidences a [[claim]] (a happening), it is about a topic and no claim follows, or it was read and there was nothing in it. Before those judgments were written, only the first left a trace — so a post correctly read and correctly found empty looked unread, and was read again on every pass.
