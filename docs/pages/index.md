...
- Prose at `docs/` root — long-form documents outl does not index (it reads only `pages/` and `journals/`), so they open in an editor, not in the app: the [clustering spike](../clustering-spike.md), the [ubiquitous language audit](../ubiquitous-language-audit.md), and the session briefs that hand a layer to a fresh session — [actor](../kickstart-actor.md), [judgments](../kickstart-judgments.md), [runs](../kickstart-runs.md); and the 2026-07-25 narrative/agent design package — the [narrative teardown](../narrative-teardown.md), the [agent architecture](../agent-architecture.md), the [extraction scenarios](../extraction-scenarios.md), and the [evaluation matrix](../evaluation-matrix.md).
- Conventions
